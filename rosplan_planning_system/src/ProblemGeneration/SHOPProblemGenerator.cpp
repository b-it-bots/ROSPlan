...
namespace KCL_rosplan {
	...
	void SHOPProblemGenerator::makeHeader(std::ofstream &pFile) {
	...
	void SHOPProblemGenerator::makeInitialState(std::ofstream &pFile) {
		...
		if (!getDomainPropsClient.call(domainAttrSrv)) {
			...
			for(; ait != domainAttrSrv.response.items.end(); ait++) {
				...
				if (!getPropsClient.call(attrSrv)) {
					ROS_ERROR("KCL: (SHOPProblemGenerator) Failed to call service %s: %s", state_proposition_service.c_str(), attrSrv.request.predicate_name.c_str());
				} else {
				...
				if (!getTILsClient.call(attrSrv)) {
					ROS_ERROR("KCL: (SHOPProblemGenerator) Failed to call service %s: %s", state_timed_knowledge_service.c_str(), attrSrv.request.predicate_name.c_str());
				} else {
	...
	void SHOPProblemGenerator::makeGoals(std::ofstream &pFile) {
		...
		if (!getCurrentGoalsClient.call(currentGoalSrv)) {
			ROS_ERROR("KCL: (SHOPProblemGenerator) Failed to call service %s", state_goal_service.c_str());
		} else {
	...
	void SHOPProblemGenerator::makeMetric(std::ofstream &pFile) {
		...
		if (!getCurrentMetricClient.call(currentMetricSrv)) {
			...
			ROS_ERROR("KCL: (SHOPProblemGenerator) Failed to call service %s", state_metric_service.c_str());
	...
	void SHOPProblemGenerator::printExpression(std::ofstream &pFile, rosplan_knowledge_msgs::ExprComposite & e) {
	...
	void SHOPProblemGenerator::makeProblem(std::ofstream &pFile) {
		makeHeader(pFile);
