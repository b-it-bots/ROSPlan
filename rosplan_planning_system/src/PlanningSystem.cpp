...
namespace KCL_rosplan {
	...
	PlanningSystem::PlanningSystem(ros::NodeHandle& nh, PlanParser& parser)
		: system_status(READY),
		  plan_parser(&parser),
	...
	{
		// dispatcher
		plan_dispatcher = new EsterelPlanDispatcher(parser);
	...
	bool PlanningSystem::runPlanner() {
		...
		while(!planfile.eof() && !solved) {
			...
			if (line.find("; Plan found", 0) != std::string::npos ||
			    line.find("ff: found legal plan as follows", 0) != std::string::npos ||
			    line.find("; Cost:", 0) != std::string::npos ||
			    line.find("Solution Found", 0) != std::string::npos)
			{
				solved = true;
				break;
			}
